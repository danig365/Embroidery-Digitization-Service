...
function TokenCostManagementContent() {
  ...
  const loadTokenCosts = async () => {
    try {
      ...
      if (data.success) {
        ...
        setMessage(`❌ ${data.error || "Failed to load token costs"}`);
      }
    } catch (error) {
      setMessage("❌ Failed to load token costs. Using defaults.");
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setMessage("✅ Token costs updated successfully");
        setOriginalCosts(costs);
        setTimeout(() => setMessage(""), 3000);
      } else {
        setMessage(`❌ ${data.error || "Failed to update token costs"}`);
      }
    } catch (error) {
      setMessage("❌ Failed to update token costs");
      console.error(error);
    } finally {
      setSaving(false);
    }
  ...
  const costItems = [
    {
      ...
      description: "Tokens spent when user generates AI images for designs",
      icon: "🤖",
    },
    {
      key: "order_placement",
      label: "Order Placement",
      description: "Tokens spent when user places an order for digitization",
      icon: "📦",
    },
  ];
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "32px" }}>
          ...
          <p style={{ fontSize: "14px", color: "#6b7280" }}>
            Control how many tokens are deducted for each action in your system
          </p>
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            ...
            <p style={{ fontSize: "16px", color: "#6b7280" }}>Loading token costs...</p>
          </div>
        ) : (
          <>
            {/* Cost Cards */}
                  ...
                  <div>
                    ...
                    {costs[item.key] !== originalCosts[item.key] && (
                      <p style={{ fontSize: "11px", color: "#f59e0b", marginTop: "6px", margin: "6px 0 0 0" }}>
                        Changed from {originalCosts[item.key]} → {costs[item.key]}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "12px" }}>
                {costItems.map((item) => (
                  <div key={item.key} style={{ fontSize: "13px", color: "#1e40af" }}>
                    ...
                    <span style={{ fontSize: "18px", fontWeight: "700", color: "#667eea" }}>{costs[item.key]}</span> tokens
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginTop: "32px", padding: "16px", background: "#fef3c7", borderRadius: "8px", border: "1px solid #fcd34d" }}>
              ...
              <ul style={{ fontSize: "13px", color: "#92400e", margin: "0", paddingLeft: "20px" }}>
                <li>These token costs are deducted from user accounts when they perform actions</li>
                <li>Adjust costs to control your token economy and user spending patterns</li>
                <li>Users cannot perform actions if they don't have enough tokens</li>
