...
def create_design(request):
    """
    Create a new design - supports both image upload and AI generation
    Costs 2 tokens for AI generation, 0 tokens for upload only
    ...
    try:
        ...
        # Create design object with ALL fields
        ...
        if uploaded_image:
            design.uploaded_image = uploaded_image
            design.save()
            
            return Response({
                "success": True,
                "message": "Design created with uploaded image",
...
def generate_embroidery_preview_new(request):
    ...
    try:
        ...
        # Check if either prompt (AI) or uploaded_image (local upload) exists
        if not design.prompt and not design.uploaded_image:
            return Response(
                {"error": "Design must have either an AI prompt or an uploaded image"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # If AI-generated design: use prompt
        # If uploaded image: use local image processing for accurate preview
        if design.prompt:
            # AI design: use OpenAI to generate embroidery style
            ...
            # Uploaded image: process locally to preserve original image
            print(f"\n🎨 CREATING LOCAL EMBROIDERY PREVIEW...")
            try:
                from .utils.image_processor import ImageProcessor
                
                processor = ImageProcessor()
                uploaded_image_path = design.uploaded_image.path
                print(f"   Loading image from: {uploaded_image_path}")
                
                # Load the uploaded image
                img = processor.load_image(uploaded_image_path)
                print(f"   ✅ Image loaded: {img.size}")
                
                # Create embroidery-style preview
                print(f"   Creating embroidery style preview...")
                embroidery_img = processor.create_embroidery_preview(img)
                print(f"   ✅ Preview created")
                
                # No need to call OpenAI, we've processed it locally
                result = {
                    'success': True,
                    'local_image': embroidery_img
                }
            except Exception as e:
                print(f"   ❌ Error creating local preview: {str(e)}")
                import traceback
                traceback.print_exc()
                return Response(
                    {"error": f"Failed to create embroidery preview: {str(e)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
...
def update_design(request, design_id):
    ...
    try:
        ...
        if 'text_content' in request.data:
            design.text_content = request.data['text_content']
        if 'text_font' in request.data:
            design.text_font = request.data['text_font']
        if 'text_style' in request.data:
            design.text_style = request.data['text_style']
        if 'text_size' in request.data:
            design.text_size = int(request.data['text_size'])
        if 'text_color' in request.data:
            design.text_color = request.data['text_color']
        if 'text_outline_color' in request.data:
            design.text_outline_color = request.data['text_outline_color']
        if 'text_outline_thickness' in request.data:
            design.text_outline_thickness = int(request.data['text_outline_thickness'])
        if 'text_position_x' in request.data:
            design.text_position_x = int(request.data['text_position_x'])
        if 'text_position_y' in request.data:
            design.text_position_y = int(request.data['text_position_y'])
        
        # Thread Colors
        if 'thread_colors' in request.data:
            import json
            thread_colors = request.data['thread_colors']
            design.thread_colors = json.loads(thread_colors) if isinstance(thread_colors, str) else thread_colors
        if 'thread_brand' in request.data:
            design.thread_brand = request.data['thread_brand']
        if 'thread_notes' in request.data:
            design.thread_notes = request.data['thread_notes']
        
        # Embroidery Settings
        if 'stitch_type' in request.data:
            design.stitch_type = request.data['stitch_type']
        if 'stitch_density' in request.data:
            design.stitch_density = int(request.data['stitch_density'])
        if 'auto_trim' in request.data:
            design.auto_trim = request.data['auto_trim']
        if 'underlay' in request.data:
            design.underlay = request.data['underlay']
        if 'jump_trim' in request.data:
            design.jump_trim = request.data['jump_trim']
        
        # Canvas/Design Settings
        if 'design_width' in request.data:
            design.design_width = int(request.data['design_width'])
        if 'design_height' in request.data:
            design.design_height = int(request.data['design_height'])
        if 'hoop_size' in request.data:
            design.hoop_size = request.data['hoop_size']
        if 'rotation' in request.data:
            design.rotation = int(request.data['rotation'])
        if 'mirror_horizontal' in request.data:
            design.mirror_horizontal = request.data['mirror_horizontal']
        if 'mirror_vertical' in request.data:
            design.mirror_vertical = request.data['mirror_vertical']
...
def generate_ai_image(request):
    ...
    result = openai_service.generate_dual_images(
        prompt, style=style, size="1024x1024", quality="high"
    )
    ...
    if event["type"] == "checkout.session.completed":
        ...
        try:
            ...
            TokenTransaction.objects.create(
                ...
                type="purchase",
                amount=tokens,
                description=f"Purchased {package.name} via Stripe",
            )
            ...
            print(
                f"✅ Payment successful: User {user.username} received {tokens} tokens"
            )
